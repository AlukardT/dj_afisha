...
def fetch_events():
    try:
        ...
        for item in items[:5]:  # ограничим 5 событиями
            # Изображение
            img_tag = item.select_one('.poster__img')
            img_src = img_tag.get('src') if img_tag else ''
            if img_src and not img_src.startswith('http'):
                img_src = 'https://dj.ru' + img_src
            ...
            if place_tag:
                # убираем переносы строк, оставляем текст
                place = place_tag.get_text(' ', strip=True)
            else:
                place = ''

            # Диджей (всегда один, но можно извлечь)
            dj_tag = item.select_one('.poster__info-i_type_dj a')
